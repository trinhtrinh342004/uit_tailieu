...
/*Câu a*/
SET DEFINE OFF;
CREATE OR REPLACE TRIGGER TRIGGER_STUDENT_dk
AFTER INSERT OR UPDATE ON ENROLLMENT
FOR EACH ROW
DECLARE
    DEMMON NUMBER;
BEGIN
    SELECT COUNT(*) INTO DEMMON
...
END;
/*Câu b*/
CREATE OR REPLACE PROCEDURE HAMINMH(MAMH IN COURSE.COURSENO%TYPE)
AS
    V_COUNT NUMBER;
    CUR_CLASSID CLASS.CLASSID%TYPE;
    V_COURSENAME COURSE.DESCRIPTION%TYPE;
    CURSOR CUR IS SELECT CLASS.CLASSID
                    FROM CLASS
                    WHERE CLASS.COURSENO = MAMH;
BEGIN
    SELECT COURSE.DESCRIPTION
    INTO V_COURSENAME
    FROM COURSE
    WHERE COURSE.COURSENO = MAMH;
    DBMS_OUTPUT.PUT_LINE('** MON HOC: '||V_COURSENAME||' (MAMH: '||MAMH||')'); 
    OPEN CUR;
    LOOP
      FETCH CUR INTO CUR_CLASSID;
      EXIT WHEN CUR%NOTFOUND;
      
      SELECT COUNT(*)
      INTO V_COUNT
      FROM ENROLLMENT E
      WHERE E.CLASSID = CUR_CLASSID;
      DBMS_OUTPUT.PUT_LINE('----- LOP: '||CUR_CLASSID||' CO SO LUONG SINH VIEN DANG KI LA: '||V_COUNT);
    END LOOP;
END;
SELECT *FROM CLASS
DECLARE 
...
/*Câu c*/
create or replace FUNCTION Total_cost_for_student(i_mastudent STUDENT.STUDENTID%TYPE)
RETURN NUMBER
AS 
    out_TONGCOST COURSE.COST%TYPE;
BEGIN
    SELECT SUM(COST)
    INTO OUT_TONGCOST
    FROM COURSE,STUDENT,CLASS,ENROLLMENT
    WHERE COURSE.COURSENO = CLASS.COURSENO AND CLASS.CLASSID =ENROLLMENT.CLASSID AND ENROLLMENT.STUDENTID = STUDENT.STUDENTID AND STUDENT.STUDENTID =i_mastudent;
    return out_TONGCOST;
    exception
...
DECLARE
BEGIN
  DBMS_OUTPUT.PUT_LINE(Total_cost_for_student(2));
END;
