...
INSERT INTO LOP VALUES ('K11','Lop 1 Khoa 1', NULL,'11','GV07') --K1108
INSERT INTO LOP VALUES ('K12','Lop 2 Khoa 1', NULL,'12','GV09') --K1205
INSERT INTO LOP VALUES ('K13','Lop 3 Khoa 1', NULL,'12','GV14') --K1305
...
--I.1 
ALTER TABLE HOCVIEN
ADD GHICHU VARCHAR(40), DIEMTB NUMERIC(4,2), XEPLOAI VARCHAR(10)
...
AS
	BEGIN
		DECLARE  @SISO tinyint, @MALOP char(3), @MAHV char(5)
		...
		SELECT @SISO= (SELECT SISO FROM LOP WHERE @MALOP = MALOP)
...
CREATE TRIGGER trg_update_HOCVIEN
ON HOCVIEN
FOR UPDATE
AS 
	BEGIN
...
CREATE TRIGGER trg_delete_HOCVIEN
ON HOCVIEN
FOR delete
AS 
	BEGIN
...
--I.3
ALTER TABLE HOCVIEN
ADD CONSTRAINT CHECK_GIOITINH_HV CHECK (GIOITINH IN ('Nam', 'Nu'))

ALTER TABLE GIAOVIEN
ADD CONSTRAINT CHECK_GIOITINH_GV CHECK (GIOITINH IN ('Nam', 'Nu'))

--I.4
ALTER TABLE KETQUATHI 
ADD CONSTRAINT CHECK_KQT CHECK
(
...
--I.5
ALTER TABLE KETQUATHI 
ADD CONSTRAINT CHECK_KQUA CHECK
(
...
--I.8 
ALTER TABLE GIAOVIEN
ADD CONSTRAINT CHECK_HOCVI CHECK
(
	HOCVI = 'CN' OR HOCVI = 'KS' OR HOCVI = 'Ths' OR HOCVI ='TS' OR HOCVI = 'PTS'
)

--I.11
ALTER TABLE HOCVIEN 
ADD CONSTRAINT CHECK_NGSINH CHECK (YEAR(GETDATE())- YEAR(NGSINH) >=18)
...
--I.13
ALTER TABLE GIAOVIEN
ADD CONSTRAINT CHECK_TUOI CHECK (YEAR(NGVL)-YEAR(NGSINH) >=22)

--I.14
ALTER TABLE MONHOC
ADD CONSTRAINT CHECK_TC CHECK(TCLT - TCTH <=3)
...
FROM KETQUATHI 
GROUP BY MAHV, MAMH) AS B, KETQUATHI KQ 
WHERE B.MAHV = KQ.MAHV AND MAX_LANTHI = KQ.LANTHI
GROUP BY B.MAHV
...
--III.2
SELECT HV.MAHV, HO, HV.TEN, KQT.LANTHI, KQT.DIEM
FROM HOCVIEN HV,KETQUATHI KQT, MONHOC MH
WHERE  HV.MAHV= KQT.MAHV AND MH.MAMH = KQT.MAMH 
AND MH.MAMH='CTRR'
AND HV.MALOP='K12'
ORDER BY HV.TEN ASC 
...
FROM HOCVIEN HV,KETQUATHI KQT, MONHOC MH
WHERE HV.MAHV=KQT.MAHV AND MH.MAMH = KQT.MAMH 
AND HV.MALOP='K11' AND KQT.KQUA='Khong Dat' 
AND KQT.LANTHI=1 AND MH.MAMH ='CTRR'
...
EXCEPT
	...
	FROM HOCVIEN HV, KETQUATHI KQT
	WHERE HV.MAHV =KQT.MAHV 
	AND KQT.MAMH ='CTRR' AND KQUA ='Dat'

--III.6
SELECT DISTINCT MAMH
FROM GIANGDAY PC,GIAOVIEN GV
...
AND HOCKY = 1 AND NAM =2006 AND MAGV =
		(
			SELECT MAGVCN
			FROM LOP
			WHERE LOP.MALOP ='K11'
...
WHERE HV.MAHV = LOP.TRGLOP AND GV.MAGV = LOP.MAGVCN AND GV.MAGV =GD.MAGV AND GD.MAMH = MH.MAMH 
AND GV.HOTEN ='Nguyen To Lan' AND TENMH='Co so du lieu'

--III.9
SELECT DIEUKIEN.MAMH_TRUOC, TENMH
FROM MONHOC,DIEUKIEN
WHERE MONHOC.MAMH = DIEUKIEN.MAMH_TRUOC AND DIEUKIEN.MAMH='CSDL'
...
AND MAMH = 'CTRR' AND HOCKY = '1' AND NAM = 2006
AND MALOP = 'K11'
INTERSECT 
SELECT HOTEN
FROM GIAOVIEN GV, GIANGDAY GD
...
WHERE KETQUATHI.MAHV=HOCVIEN.MAHV AND KQUA ='Khong Dat' AND MAMH='CSDL'
EXCEPT
SELECT HOCVIEN.MAHV, HOCVIEN.HO, HOCVIEN.TEN
FROM KETQUATHI, HOCVIEN
WHERE KETQUATHI.MAHV=HOCVIEN.MAHV AND LANTHI>1 AND MAMH='CSDL'
...
FROM HOCVIEN  LEFT JOIN KETQUATHI ON HOCVIEN.MAHV = KETQUATHI.MAHV
...
--III.18
SELECT HOCVIEN.MAHV,HO,TEN,MAX(DIEM) AS DIEMCSDL
FROM HOCVIEN, KETQUATHI
WHERE HOCVIEN.MAHV = KETQUATHI.MAHV AND MAMH = 'CSDL'
GROUP BY HOCVIEN.MAHV,HOCVIEN.TEN,HOCVIEN.HO
...
--III.20
SELECT COUNT(MAGV) 'So giao vien'
FROM GIAOVIEN
WHERE HOCHAM IN ('GS', 'PGS')

--III.21
SELECT K.MAKHOA, COUNT(MAGV) 'So giao vien'
...
--III.22
SELECT MAMH, KQUA, count(distinct MAHV) N'SO LUONG'
FROM KETQUATHI KQ1
WHERE LANTHI = (
					SELECT MAX(LANTHI) 
					FROM KETQUATHI KQ2
...
WHERE GV.MAGV=GD.MAGV and GD.MALOP=L.MALOP and GV.MAGV=L.MAGVCN
...
WHERE LOP.TRGLOP=HOCVIEN.MAHV 
AND HOCVIEN.MAHV=kq1.MAHV AND kq1.KQUA='Khong Dat' 
AND LANTHI = (
					SELECT MAX(LANTHI) 
					FROM KETQUATHI kq2 
...
--III.26
SELECT  HO,TEN,COUNT(DIEM) AS DEM_DIEM
...
--III.28
SELECT GIAOVIEN.MAGV,HOTEN,HOCKY, NAM,COUNT(MALOP) AS SOLOPDAY, COUNT(DISTINCT(MAMH)) AS SOMONDAY
...
FROM GIAOVIEN JOIN GIANGDAY ON GIAOVIEN.MAGV = GIANGDAY.MAGV
GROUP BY GIAOVIEN.MAGV,HOCKY,NAM)

--III.30
SELECT MONHOC.MAMH,TENMH,COUNT(MAHV) AS KHONGDATMAX
FROM MONHOC JOIN KETQUATHI ON MONHOC.MAMH = KETQUATHI.MAMH
...
GROUP BY HOCVIEN.MAHV,HO,TEN
HAVING COUNT(DISTINCT kq1.MAMH) = (
										SELECT COUNT(*) 
										FROM MONHOC
								  )

--III.35
SELECT DISTINCT HV.MAHV,HO,TEN,MAMH,MAX(DIEM) AS MAXDIEM
FROM (SELECT MAHV,MAMH,DIEM,MAX(LANTHI) AS MAXLANTHI
	FROM KETQUATHI
	GROUP BY MAHV,MAMH,DIEM) AS KQM JOIN HOCVIEN HV ON KQM.MAHV = HV.MAHV
GROUP BY HV.MAHV,HO,TEN,MAMH
HAVING MAX(DIEM) >=ALL(SELECT MAX(DIEM) AS MAXDIEM
FROM (SELECT MAHV,MAMH,DIEM,MAX(LANTHI) AS MAXLANTHI
	FROM KETQUATHI
	GROUP BY MAHV,MAMH,DIEM) AS KQM JOIN HOCVIEN HV ON KQM.MAHV = HV.MAHV
GROUP BY HV.MAHV,HO,TEN,MAMH)
