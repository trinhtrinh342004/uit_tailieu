...
--3c
SELECT HOTEN, COUNT(MADA) 'So de an'
...
WHERE PC.MADA = DA.MADA AND PB.MAPHG = DA.MAPHG AND NV.MANV = PC.MANV
EXCEPT (
			SELECT DISTINCT HOTEN
			FROM NHANVIEN NV, PHONGBAN PB, DEAN DA, PHANCONG PC
			WHERE PC.MADA = DA.MADA AND PB.MAPHG = DA.MAPHG AND NV.MANV = PC.MANV
...
WHERE NOT EXISTS(
					...
					AND NOT EXISTS (
									...
									AND DA.MADA = PHANCONG.MADA
									)
		  )

--3f
SELECT PB.TENPHG, NV.HOTEN AS [Ten truong phong]
